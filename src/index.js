...
/**
 * Vega provider for YouTube, backed by Piped's public API.
 * The methods intentionally return plain objects so they can be consumed by
 * Vega without coupling the provider to a particular UI implementation.
 */
class YouTubeVegaProvider {
  ...
  async request(path) {
    ...
    if (!response.ok) {
      throw new Error(`Piped request failed with HTTP ${response.status}`);
    }
  ...
  async search(query) {
    ...
    try {
      ...
      return items
        .filter((item) => item && (item.url || item.id || item.videoId))
        .map((item) => {
          ...
          return {
            ...
            poster: item.thumbnail || item.thumbnailUrl || null,
            type: "movie",
            description: item.uploaderDescription || `Uploaded by ${uploader}`,
            uploader,
          };
        })
        .filter((item) => item.id);
  ...
  async getStreams(videoId) {
    ...
    try {
      ...
      if (typeof hls === "string" && /\.m3u8(?:$|\?)/i.test(hls)) {
        streams.push({ name: "HLS Stream", url: hls, quality: "Auto", type: "hls" });
      }
      ...
      for (const stream of Array.isArray(data.videoStreams) ? data.videoStreams : []) {
        ...
        if (isMp4 && stream.url) {
          streams.push({
            name: `Direct MP4 (${stream.quality || stream.videoOnly ? "Available" : "Auto"})`,
            url: stream.url,
  ...
  // Alias used by some Vega provider loaders.
  async streams(videoId) {
    return this.getStreams(videoId);
  }
}
